...
## Leaderboards
`rwg-leaderboard.js` is the only browser leaderboard client. Rankings are scoped by the server-whitelisted tuple `(gameSlug, variantSlug, seasonSlug, scoreVersion)`; games without variants use `default`, while Solitario uses `klondike`/`freecell`. The public scoring catalog selects one current season per game/variant and retains registered historical seasons without mixing their rows. Current Solitario and Neon Tilt use `scoring-v2`/version 2; games awaiting their scoring rollout remain `legacy-v1`/version 1. Unversioned submissions for a game that has moved to v2 are conservatively assigned to legacy v1. Games and pause code communicate through shared lifecycle events and never call leaderboard HTTP endpoints directly. Variant selectors emit `rwg:leaderboard-scope-change`; run ids, cache and pagination are scope-local through the scoring season. A run id spans unfinished-session restore and credit Continue, while a true new game receives a new id. Offline submission remains idempotently queued with its original immutable scoring scope.
...
Catalogs use stable semantic namespaces and named placeholders. Translated text is rendered at the edge and never persisted in authoritative game state. Astro static output is the production build-time generator for the complete IT/EN route sets; game engines, shared lifecycle ordering, Fastify and Nginx ownership remain unchanged. `RWGI18n` loads synchronously before shared/game bootstrap, with a shared-only bundle on non-game pages and one self-contained shared-plus-game bundle on each game route. `scripts/validate-i18n.mjs` is part of the repository-wide contract and incomplete locales remain non-publishable.
