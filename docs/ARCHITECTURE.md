...
The home owns the install-acquisition layer. `pwa-install.js` captures `beforeinstallprompt`, shows the first-visit notice after 2 seconds only once, controls the persistent end-of-list card, hides both surfaces in standalone mode and provides an explicit iOS system-instruction fallback. `sw.js` is root-scoped and network-first, caching successful same-origin responses only for offline fallback. UI lives in `pwa-install.css`; games and shared Game Over must not duplicate install prompts. See `docs/PWA-INSTALL.md`.
