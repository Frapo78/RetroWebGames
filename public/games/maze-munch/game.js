(() => {
'use strict';
const t = (key, params = {}) => window.RWGI18n.t(key, params);const M=window.MM,c=M.dom.canvas;
...
M.dom.pause.addEventListener('click',()=>{if(!M.running)return;M.paused=!M.paused;M.dom.pause.textContent=M.paused?'▶':'Ⅱ';M.status(M.paused?t('games.mazeMunch.pause'):t('games.mazeMunch.hunting'));});
...
document.addEventListener('visibilitychange',()=>{if(document.hidden&&M.running&&!M.paused){M.paused=true;M.dom.pause.textContent='▶';M.status(t('games.mazeMunch.pause'));}});
window.addEventListener('rwg:continue-game',e=>{M.score=Math.max(0,Math.floor(e.detail?.score??M.score));M.lives=1;M.running=true;M.paused=false;M.dom.pause.textContent='Ⅱ';M.dom.start.textContent=t('core.replay');M.dom.overlay.classList.remove('visible');M.resetActors(2.8);M.hud();M.last=performance.now();M.status(t('games.mazeMunch.continue'));M.tone(520,.16,'triangle',.035,900);});
