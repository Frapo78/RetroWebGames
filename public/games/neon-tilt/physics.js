(() => {
  ...
  function circleHit(a, b, extra = 0) {
    const rr = extra < 0 ? Math.max(.19, a.r + b.r + extra - .38) : a.r + b.r + extra;
    const dx = a.x - b.x;
    const dy = a.y - b.y;
  ...
  function step(ball, world, input, dt, difficulty = 1) {
    const maxDt = 1 / 45;
    const chunks = Math.max(1, Math.ceil(dt / maxDt));
    const sub = dt / chunks;
    let wallHit = false;
    let bumperHit = null;
    ...
    for (let n = 0; n < chunks; n++) {
      ...
      let ax = input.x * accelScale;
      let ay = input.y * accelScale;
      const boost = world.boostAt(ball.x, ball.y);
      if (boost) {
        ax += boost.x * 15.5;
        ay += boost.y * 15.5;
      }
      ...
      ball.vx += ax * sub;
      ball.vy += ay * sub;
      const damping = Math.exp(-friction * sub);
      ball.vx *= damping;
      ball.vy *= damping;

      const maxSpeed = tile === '~' ? 8.0 : 6.6 + Math.min(1.2, difficulty * .25);
      ...
      ball.x += ball.vx * sub;
      ball.y += ball.vy * sub;
      for (const wall of world.walls) if (resolveCircleRect(ball, wall)) wallHit = true;
      for (const bumper of world.bumpers) if (resolveBumper(ball, bumper)) bumperHit = bumper;
    }
