(() => {
  ...
  const overlay=document.createElement('section');overlay.id='rwgPauseMenu';overlay.className='rwg-pause-menu';overlay.hidden=true;overlay.setAttribute('role','region');overlay.setAttribute('aria-labelledby','rwgPauseTitle');overlay.innerHTML=`<div class="rwg-pause-card"><div class="rwg-pause-kicker">${t('pause.kicker', { game: gameName.toUpperCase() })}</div><h2 id="rwgPauseTitle">${t('pause.title')}</h2><p class="rwg-pause-copy" data-rwg-pause-copy>${t('pause.safe')}</p><div class="rwg-pause-stats" data-rwg-pause-stats></div><div class="rwg-pause-actions" data-rwg-pause-actions><button type="button" class="rwg-pause-resume" data-rwg-pause-resume>▶ <span>${t('pause.resume')}</span></button><button type="button" class="rwg-pause-end" data-rwg-pause-end>■ <span>${t('pause.end')}</span></button></div><div class="rwg-pause-confirm" data-rwg-pause-confirm hidden></div></div>`;document.body.appendChild(overlay);
  ...
  const renderStats=()=>{const i=eligibility();stats.innerHTML=`<div><span>${t('pause.score')}</span><strong>${window.RWGI18n.number(i.score)}</strong></div><div><span>${t('pause.activeTime')}</span><strong>${formatDuration(i.activeMs)}</strong></div>`;};
  const resetConfirm=()=>{confirm.hidden=true;confirm.replaceChildren();actions.hidden=false;copy.textContent=t('pause.safe');};
  ...
  const finalizeTermination=()=>{
    ...
    document.documentElement.classList.remove('rwg-shared-pause-open');document.documentElement.classList.add('rwg-pause-finalizing');actions.hidden=true;confirm.hidden=false;confirm.innerHTML=`<p class="rwg-pause-finalizing">${info.eligible?t('pause.registering'):t('pause.ended')}</p>`;
  ...
  const showConfirmation=stage=>{actions.hidden=true;confirm.hidden=false;const info=eligibility(),eligibilityCopy=info.eligible?t('pause.eligible', { score: window.RWGI18n.number(info.score), duration: formatDuration(info.activeMs) }):t('pause.ineligible', { score: window.RWGI18n.number(policy.minScoreExclusive) });if(stage===1){copy.textContent=t('pause.warning');confirm.innerHTML=`<p>${eligibilityCopy}</p><div class="rwg-pause-confirm-actions"><button type="button" data-rwg-cancel>${t('core.cancel')}</button><button type="button" class="danger" data-rwg-next>${t('pause.terminate')}</button></div>`;confirm.querySelector('[data-rwg-cancel]').addEventListener('click',resetConfirm);confirm.querySelector('[data-rwg-next]').addEventListener('click',()=>showConfirmation(2));}else{copy.textContent=t('pause.finalWarning');confirm.innerHTML=`<p>${t('pause.finalCopy')}</p><div class="rwg-pause-confirm-actions"><button type="button" data-rwg-back>${t('core.back')}</button><button type="button" class="danger strong" data-rwg-final>${t('pause.final')}</button></div>`;confirm.querySelector('[data-rwg-back]').addEventListener('click',()=>showConfirmation(1));confirm.querySelector('[data-rwg-final]').addEventListener('click',finalizeTermination);}};
