(() => {
  ...
  const eligibility=()=>{const state=snapshot(),score=scoreFrom(state),duration=Math.max(activeMs,stateDurationMs(state));return{state,score,level:levelFrom(state),activeMs:duration,eligible:score>policy.minScoreExclusive&&duration>=MIN_ACTIVE_MS};};
  ...
  const terminalDetail=info=>{const state=info.state||{},metrics={terminalReason:'pause-terminate',interrupted:true};for(const key of ['moves','lines','foods','combo','playerScore','cpuScore','rally'])if(Number.isFinite(Number(state[key])))metrics[key]=Number(state[key]);return{game:gameName,gameSlug,outcome:'game-over',score:info.score,level:info.level,activeMs:Math.round(info.activeMs),continueCount:0,achievements:[],terminalReason:'pause-terminate',metrics};};
  ...
  const finalizeTermination=()=>{
    ...
    window.addEventListener('rwg:leaderboard-registered',onRegistered);window.dispatchEvent(new CustomEvent('rwg:leaderboard-result',{detail}));setTimeout(finish,5000);
  };
  ...
  syncRun();sync();window.RWGPauseMenu=Object.freeze({minActiveMs:MIN_ACTIVE_MS,policy:Object.freeze({...policy}),eligibility,sync});
})();