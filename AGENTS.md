...
Block Drop had a cold-load race where the asynchronous shared joystick changed the stage after the engine measured its Canvas. Its renderer must retain a Canvas-owned `ResizeObserver`, coalesced atomic backing-store/DPR/cell updates and stored render dimensions; `draw()` must never mix a live CSS rectangle with stale geometry. Rotate and Drop remain square, rounded, adjacent icon-led actions. Its orange foreground cover hero is always a legal four-block T tetromino, never a decorative impossible polyomino. See `docs/BLOCK-DROP.md` and `scripts/validate-block-drop.mjs`; do not regress to a one-shot window resize or unversioned local assets.
...
- I18N-3/4 make Astro the authoritative static generator for the complete IT/EN/ES route sets. Run `npm run publish:site` before repository-wide validation whenever source HTML, catalogs, route metadata or the renderer changes.
- Every asset, stylesheet, script and lazy-image URL emitted under a prefixed locale MUST be root-absolute (or fully qualified). Relative `src`, `href`, `data-rwg-src` and `data-rwg-srcset` values resolve below `/en/` or `/es/` and previously broke Home artwork, leaderboard boot and shared HUD controls. Dynamic game assets such as Bubble Burst sprite sheets follow the same rule. `validate-i18n.mjs` is the regression gate.
- `scripts/generate-sitemap.mjs` is also invoked by the VPS update workflow and MUST remain byte-equivalent to the Astro sitemap finalizer. It must preserve the exact reciprocal `it`, `en`, `es` and `x-default` URL set for every indexable route; never reduce validation to checking only that an `hreflang` token exists.
- Internal navigation inherits the locale encoded in the current route, which has priority over browser `Accept-Language`. Every Home game card and shared back/menu link on `/en/` or `/es/` must target the corresponding prefixed route; do not leave canonical Italian absolute URLs in localized navigation.
...
Completed Solitario hands use only the versioned `games/solitaire/scoring.js` formula. Preserve distinct Klondike/FreeCell time and move targets, the exact inclusive `1..10,000` bounds, monotonic time/move efficiency and non-rewindable Hint/Undo penalties. Score `0` is exclusively a zero-move abandonment and must never be submitted; the server must reject Solitario scores outside `1..10,000`. Do not return to the legacy `1000 - elapsed * 2` bonus or let Undo restore assistance counters; leaderboard metrics must retain `scoringVersion`, `hintsUsed` and `undosUsed`.
