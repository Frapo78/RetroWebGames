(() => {
  ...
  const base = () => ({
    page_kind: pageKind,
    ...(isGame ? { game_id: gameId, game_name: gameName } : {})
  });
  ...
  let visibleStartedAt = document.hidden ? 0 : performance.now();
  ...
  function beginEngagement(reason) {
    gameplayActive = true;
    if (!document.hidden && !visibleStartedAt) visibleStartedAt = performance.now();
  ...
  function onReady() {
    ...
    if (isGame) {
      ...
      startBtn?.addEventListener('click', () => {
        const restart = gameEnded || gameStarted;
        gameStarted = true;
        gameEnded = false;
        accumulatedVisibleMs = 0;
        visibleStartedAt = document.hidden ? 0 : performance.now();
        engagementMilestones.clear();
        track(restart ? 'game_restart' : 'game_start', { variant: variantName() });
        beginEngagement(restart ? 'restart' : 'new');
      }, true);
  ...
  document.addEventListener('click', event => {
    ...
    if (share) {
      ...
      track('share', {
        method,
        content_type: isGame ? 'game' : 'site',
        item_id: isGame ? gameId : 'retrowebgames'
      });
    }
    ...
    if (target.id === 'pauseBtn') {
      queueMicrotask(() => track('game_pause_toggle', { paused: bool(target.textContent.trim() === '▶') }));
    }

    const controlMap = {
      undoBtn: 'undo', hintBtn: 'hint', newDealBtn: 'new_deal', calibrateBtn: 'calibrate', muteBtn: 'mute', cardStyleSelect: 'card_style'
    };
  ...
  window.addEventListener('rwg:session-declined', () => track('game_resume_declined'));
  window.addEventListener('rwg:session-restore-failed', () => track('game_resume_failed'));
