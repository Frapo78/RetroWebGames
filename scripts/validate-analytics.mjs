...
for (const marker of [
  ...
  "track(restart ? 'game_restart' : 'game_start'",
  "track('game_resume'",
  "track('game_continue'",
  "track('level_reached'",
...
for (const rel of ['index.html', 'avatar/index.html', ...gameDirs.map(e => `games/${e.name}/index.html`).filter(rel => fs.existsSync(path.join(root, rel)))]) {
  const html = read(rel);
  must(!html.includes('G-ZSWLC4L8GW') || rel === 'index.html' || rel === 'avatar/index.html' || !html.includes("gtag('config'"), `${rel}: GA config should remain centralized in rwg-analytics.js`);
}
...
console.log('  ✓ gameplay funnel, resume, engagement, share and install events present');
