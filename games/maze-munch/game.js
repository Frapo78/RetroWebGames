...
window.addEventListener('rwg:continue-game',e=>{M.score=Math.max(0,Math.floor(e.detail?.score??M.score));M.lives=1;M.running=true;M.paused=false;M.dom.pause.textContent='Ⅱ';M.dom.start.textContent='RIGIOCA';M.dom.overlay.classList.remove('visible');M.resetActors(2.8);M.hud();M.last=performance.now();M.status('CONTINUA!');M.tone(520,.16,'triangle',.035,900);});
