(() => {
  ...
  function shoot() {
    ...
    moving = {
      x: launcherX + v.x * (R + 16),
      y: launcherY + v.y * (R + 16),
      vx: v.x * speed,
      vy: v.y * speed,
      color: currentColor
    };
  ...
  function component(startR, startC, color = null) {
    ...
    while (queue.length) {
      ...
      for (const [nr, nc] of neighbors(r, c)) {
        ...
        if (!seen.has(nk) && nb && nb.color === target) {
          seen.add(nk);
          queue.push([nr, nc]);
        }
      }
  ...
  function dropDisconnected() {
    ...
    while (queue.length) {
      ...
      for (const [nr, nc] of neighbors(r, c)) {
        const nk = key(nr, nc);
        if (grid.has(nk) && !anchored.has(nk)) {
          anchored.add(nk);
          queue.push([nr, nc]);
        }
      }
  ...
  function addPenaltyRow() {
    ...
    for (const b of grid.values()) {
      const nr = b.r + 1;
      if (validCell(nr, b.c)) shifted.set(key(nr, b.c), { r: nr, c: b.c, color: b.color });
    }
    grid.clear();
    for (const [k, b] of shifted) grid.set(k, b);

    for (let c = 0; c < COLS; c++) {
      if (!validCell(0, c)) continue;
      const color = PALETTE[Math.floor(Math.random() * colorCount)];
      grid.set(key(0, c), { r: 0, c, color });
    }

    misses = 0;
    banner = 'NUOVA RIGA!';
    bannerTime = 1.1;
    tone(135, .16, 'sawtooth', .035, -55);
    if (navigator.vibrate) navigator.vibrate(35);
  }

  function reconcileQueue() {
    const colors = activeColors();
    if (!colors.includes(currentColor)) currentColor = pickColor();
    if (!colors.includes(nextColor)) nextColor = pickColor();
  }

  function resolveShot(cell) {
    const matches = component(cell.r, cell.c);
    if (matches.length >= 3) {
      removeCells(matches, 35);
      const dropped = dropDisconnected();
      score += Math.max(0, matches.length - 3) * 15;
      tone(640, .09, 'triangle', .04, 280);
      if (dropped) tone(330, .14, 'sine', .035, -110);
      misses = 0;
    } else {
      misses++;
      tone(210, .04, 'square', .018, -35);
      if (misses >= missLimit) addPenaltyRow();
    }
  ...
  function attachMoving() {
    ...
    moving = null;
    grid.set(key(cell.r, cell.c), { r: cell.r, c: cell.c, color });
    burst(p.x, p.y, color, 5, 65);
    tone(360, .035, 'sine', .018, 60);
    resolveShot(cell);
  }
  ...
  function nextLevel() {
    level++;
    misses = 0;
    missLimit = Math.max(3, 5 - Math.floor((level - 1) / 3));
    colorCount = Math.min(PALETTE.length, 4 + Math.floor(level / 2));
    score += 750;
    spawnBoard();
    currentColor = pickColor();
    nextColor = pickColor();
    banner = `LIVELLO ${level}`;
    bannerTime = 1.5;
    tone(480, .18, 'triangle', .045, 480);
    updateHud();
  }
  ...
  function checkDanger() {
    ...
    for (const b of grid.values()) {
      if (cellPos(b.r, b.c).y + R >= dangerY) {
        endGame();
        return true;
      }
    }
  ...
  function endGame() {
    ...
    localStorage.setItem('bubbleBurstBest', String(best));
    updateHud();
  ...
  function updateMoving(dt) {
    ...
    if (moving.x <= R) {
      ...
      tone(290, .025, 'square', .012, 30);
    } else if (moving.x >= W - R) {
      moving.x = W - R;
      moving.vx = -Math.abs(moving.vx);
      tone(290, .025, 'square', .012, 30);
    }
    ...
    for (const b of grid.values()) {
      const p = cellPos(b.r, b.c);
      if ((p.x - moving.x) ** 2 + (p.y - moving.y) ** 2 <= (R * 1.88) ** 2) {
        attachMoving();
        return;
      }
    }
  ...
  function drawBubble(x, y, color, radius = R) {
    ctx.save();
    ctx.translate(x, y);
    ctx.shadowBlur = Math.max(5, radius * .65);
    ...
    shade.addColorStop(0, 'rgba(255,255,255,.72)');
    shade.addColorStop(.22, 'rgba(255,255,255,.12)');
  ...
  function traceAim() {
    if (!running || paused || moving) return;
    const v = aimVector();
    let x = launcherX + v.x * (R + 20);
    let y = launcherY + v.y * (R + 20);
    let vx = v.x;
    let vy = v.y;
    const step = 13;
  ...
  function drawLauncher() {
    ...
    g.addColorStop(0, '#4a5873');
    g.addColorStop(.5, '#e7f2ff');
    g.addColorStop(1, '#53617c');
    ctx.fillStyle = g;
    ctx.fillRect(-7, -41, 14, 43);
    ctx.strokeStyle = 'rgba(101,231,255,.35)';
    ctx.strokeRect(-7, -41, 14, 43);
    ctx.restore();

    ctx.fillStyle = '#18233b';
    ctx.beginPath();
    ctx.arc(launcherX, launcherY + 3, R + 9, Math.PI, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = 'rgba(255,255,255,.18)';
    ctx.stroke();

    if (!moving) drawBubble(launcherX, launcherY - 5, currentColor, R);
  }

  function drawBackground() {
    const g = ctx.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, '#10284a');
    g.addColorStop(.48, '#081128');
    ...
    const spacing = 34;
    for (let y = TOP; y < launcherY; y += spacing) {
      for (let x = (Math.floor(y / spacing) % 2) * spacing / 2; x < W; x += spacing) {
    ...
    ctx.save();
    ctx.setLineDash([5, 8]);
    ctx.strokeStyle = 'rgba(255,95,115,.38)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ...
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = 'rgba(255,95,115,.55)';
    ctx.font = '8px ui-monospace, monospace';
    ctx.fillText('DANGER', 10, dangerY - 6);
    ctx.restore();
  }
  ...
  function draw() {
    ...
    for (const b of falling) drawBubble(b.x, b.y, b.color, R * .92);
    ...
    if (bannerTime > 0 && running) {
      ...
      ctx.fillText(banner, W / 2, H * .56);
      ctx.restore();
    }

    if (paused && running) {
      ctx.fillStyle = 'rgba(2,5,14,.58)';
      ctx.fillRect(0, 0, W, H);
      ctx.textAlign = 'center';
  ...
  function frame(ts) {
    const dt = Math.min(.033, Math.max(0, (ts - last) / 1000 || 0));
    last = ts;
    update(dt);
    draw();
  ...
  function setAim(e) {
    ...
    aimX = Math.max(R, Math.min(W - R, p.x));
    aimY = Math.min(launcherY - 40, p.y);
  }
